...
privileged aspect AppUserController_Roo_Controller {
    ...
    public String AppUserController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("appusers", AppUser.findAppUserEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) AppUser.countAppUsers() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("appusers", AppUser.findAllAppUsers(sortFieldName, sortOrder));
        }
        addDateTimeFormatPatterns(uiModel);
    ...
    void AppUserController.addDateTimeFormatPatterns(Model uiModel) {
        uiModel.addAttribute("appUser_createddate_date_format", "dd/MM/yyyy");
        uiModel.addAttribute("appUser_updateddate_date_format", "dd/MM/yyyy");
    }
